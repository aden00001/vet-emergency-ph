...
export function isHumanBiteCenter(name, category = "") {
  const n = (name || "").trim();
...
export function humanBiteExclusionReason(name, category = "") {
  return isHumanBiteCenter(name, category)
    ? "Human anti-rabies / animal-bite treatment center (not a veterinary clinic)"
    : null;
}

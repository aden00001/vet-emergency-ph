-- Allow address-only listings (no map pin) and track whether directions are reliable.

ALTER TABLE public.clinics
  ALTER COLUMN location DROP NOT NULL;

ALTER TABLE public.clinics
  ADD COLUMN IF NOT EXISTS location_verified BOOLEAN NOT NULL DEFAULT TRUE;

COMMENT ON COLUMN public.clinics.location_verified IS
  'When false, listing is address-only — hide Maps/Waze directions until coordinates are verified.';
...
BEGIN
  ...
  WHERE c.location IS NOT NULL
    AND extensions.ST_DWithin(c.location, v_origin, p_radius_km * 1000)
